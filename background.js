document.addEventListener('DOMContentLoaded', () => {
    ...
    function animateCanvas() {
        ...
        // Controllo dinamico del colore in base al tema
        const isLightMode = document.body.classList.contains('light-mode');
        const baseColor = isLightMode ? '0, 0, 0' : '255, 255, 255'; // Nero in light, Bianco in dark
        const particleColor = isLightMode ? '#070707' : '#ffffff';
        ...
        for (let i = 0; i < particles.length; i++) {
            for (let j = i; j < particles.length; j++) {
                ...
                if (distance < connectionDistance) {
                    ctx.beginPath();
                    // Applica il colore dinamico con l'opacità calcolata
