document.addEventListener("DOMContentLoaded", () => {
  // --- DEFINIZIONE MEDIA QUERY DESKTOP ---
  const desktopQuery = window.matchMedia('(min-width: 1024px)');

  // --- CONTROLLO INIZIALE RESPONSIVE ---
  // Se non siamo su desktop, non facciamo nulla e puliamo eventuali flag
  ...
  if (sessionStorage.getItem("pageTransition") === "1") {
    ...
    // Sicurezza ulteriore: se nel frattempo l'utente ha ridimensionato/cambiato orientamento
    if (!desktopQuery.matches) {
      clearFlag();
    } else {
  ...
  linksToAnimate.forEach((a) => {
    a.addEventListener("click", (ev) => {
      ...
      // ⛔⛔⛔ STOP: Se non è desktop, NON fare animazione.
