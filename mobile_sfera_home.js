...
// Variabili Rotazione (Giroscopio) - RIMOSSE
...
function animateMobile() {
    ...
    if (sphere_mob) {
        ...
        // 2. Applichiamo la rotazione (solo tempo, no giroscopio)
        // finalRotX += (targetRotX - finalRotX) * 0.1;
        // finalRotY += (targetRotY - finalRotY) * 0.1;

        const time = Date.now() * 0.001;

        sphere_mob.rotation.x = (Math.sin(time) * 0.05);
        sphere_mob.rotation.y = (-Math.PI / 2) + (Math.cos(time * 0.8) * 0.05);
